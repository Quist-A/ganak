...
void Solver::solve(const string &file_name)
{
  ...
  if (!createfromFile(file_name))
  {
    ...
    cout << endl
         << " FOUND UNSAT DURING FILE PARSING " << endl;
    stopwatch_.stop();
  ...
  {
    if (!config_.quiet && !config_.use_isocc)
  ...
  else if (!config_.quiet)
  {
    if (!config_.perform_projectedmodelcounting)
    {
      if (config_.use_isocc) {
        cerr << "ERROR: Sampling set is present but Symganak does not support projected model counting."
             << " Please remove the sampling set present in the input file starting from 'c ind '." << endl;
        exit(-1); 
      }
  ...
  if (notfoundUNSAT)
  {
    ...
    last_ccl_cleanup_time_ =
        statistics_.getTime();

    cout << "Reserving space for violated clause" << endl;
    violated_clause.reserve(num_variables());
    cout << "Done reserving space for violated clause" << endl;

    cout << "Initializing comp manager" << endl;
    comp_manager_.initialize(literals_, literal_pool_, num_variables());
    cout << "Done initializing comp manager" << endl;
  ...
  else
  {
    ...
    statistics_.set_final_solution_count(0.0);
    cout << endl
         << " FOUND UNSAT DURING PREPROCESSING " << endl;
  }
  ...
  statistics_.writeToFile("data.out");
  statistics_.printShort();
  comp_manager_.destroy();
}
//comp_manager_.writeCacheScoresToFile("cacheScores.out");
//comp_manager_.writeComponentsToFile("components.out");
//comp_manager_.writeDiffComponentsToFile("components_diff.out");
...
SOLVER_StateT Solver::countSAT()
{
  ...
  while (true)
  {
    while (comp_manager_.findNextRemainingComponentOf(stack_.top()))
    {
      ...
      if (2 * log2(t) > log2(config_.delta) + 64 * config_.hashrange * 0.984)
...
void Solver::decideLiteral()
{
  ...
  else
  {
    ...
    if (config_.use_csvsads)
    {
      ...
      //std::cout << "considered: " << considered << endl;
      max_score = score;
    }
  }
...
//#endif
  ...
  if (config_.maxdecterminate)
  {
    ...
    {
      cout << "Terminating solver because the number of decisions exceeds the given value of " << config_.maxdec << " and conflicts is less than " << config_.minconflicts_ << endl;
      exit(1);
    }
  }
  ...
  */
  if (!isindependent && config_.perform_projectedmodelcounting)
  {
    do
    {
      ...
      stack_.top().nextUnprocessedComponent();
      assert(stack_.top().remaining_components_ofs() <
             comp_manager_.component_stack_size() + 1);
    } while (stack_.get_decision_level() >= 0);
  }
  else
  {
    do
    {
      ...
      comp_manager_.cacheModelCountOf(
          stack_.top().super_component(),
          stack_.top().getTotalModelCount());

      // Cache score should be decreased since the component is getting added
      // to cache.
      if (config_.use_csvsads)
      {
        statistics_.numcachedec_++;
...
void Solver::recordLastUIPCauses()
{
  ...
  while (lits_at_current_dl)
  {
    ...
    else
    {
      ...
      if (!seen[alit.var()] && !(var(alit).decision_level == 0) && !existsUnitClauseOf(alit.var()))
      {
